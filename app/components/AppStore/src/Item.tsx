...
const Item:React.FC<ItemProps> = ({selectedId, onClose}) => {
  ...
  const { id, title, description, skill, image, worked } = check;
  ...
  return (
    <>
      ...
      ></motion.div>
        ...
        <div className="card-content-container open absolute inset-0 z-[9999]" onClick={(e) => e.stopPropagation()}>
          ...
          >
            <FlowbiteCarousel images={image} interval={4000} className="w-[700px] h-[700px] transition duration-500 ease-in-out object-cover absolute inset-[250px] left-[680px] -translate-x-1/2 -translate-y-1/2"/>
            </motion.div>
            <motion.div
            ...
            <motion.div className="content-container overflow-auto max-h-[550px] scrollbar-pixel" animate>
            <div className="flex flex-wrap gap-1 mb-1">
               {skill.map((item, index) => (
                 <div key={index} className="rounded-full text-gray-500 border border-gray-500 border-solid hover:bg-white pl-1 pr-1 m-0 cursor-pointer">
